...
do
  CMD=$( sleep 1 && $GLOBALPATH/bin/cleos.sh system newaccount $NAME $account $PUB_KEY --stake-net "10000.0000 EOS" --stake-cpu "10000.0000 EOS" --buy-ram-kbytes 1024768 --transfer 2>$tpm_stderr)
  ERR=$(cat $tpm_stderr)
